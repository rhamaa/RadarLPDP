...
#define BUFFER_SAMPLES  8192      // Per channel
#define PRE_SAMPLES     4096
#define POST_SAMPLES    4096      // PRE + POST = 8192
...
#define CODE_VERSION "Code trigger V.3 Trigger Mod PRE"
...
int main() {
    ...
    while (!exit_now) {
        ...
        card,
        WD_AI_TRGMOD_PRE,        // PRE trigger
        WD_AI_TRGSRC_ExtD,       // External trigger source
        WD_AI_TrgNegative,       // Trigger polarity
        PRE_SAMPLES,             // PRE samples (before trigger)
        0.0,                     // Level (unused)
        0, 0, 0, 1               // Sisa parameter, sama seperti POST
    );
