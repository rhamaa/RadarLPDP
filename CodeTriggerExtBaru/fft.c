...
#define CODE_VERSION "Code trigger V.3, Plus FFT"
#define AUTHOR_NAME  "Raihan Muhammad"

#define FFT_SIZE BUFFER_SAMPLES  // FFT size = panjang sample per channel (harus power of 2)
...
void save_batch_to_file() {
    ...
    if (!f_log) {
        // Error fatal saja yang pakai printf
...
// --------------- FFT UNTUK 2 CHANNEL, 1 FILE (TANPA PRINTF) -------
void process_fft_and_save_onefile(float* ch0, float* ch1, int len, const char* folder_path) {
    ...
    kiss_fft(cfg, in, out);
    for (int i = 0; i < FFT_SIZE; ++i) {
        mag0[i] = sqrt(out[i].r * out[i].r + out[i].i * out[i].i);
    }
    ...
    kiss_fft(cfg, in, out);
    for (int i = 0; i < FFT_SIZE; ++i) {
        mag1[i] = sqrt(out[i].r * out[i].r + out[i].i * out[i].i);
    }
    ...
    // Tidak ada printf sama sekali di sini
...
int main() {
    ...
    // Bagian berikut hanya info sekali saat start
    ...
    while (!exit_now) {
        ...
        // ------------------- TULIS FILE LIVE DENGAN TEMPORARY FILE ------------------------
        ...
        if (!f_out_live) {
            // error fatal saja
            printf("Gagal membuka file live UI: %s\n", live_filepath_tmp);
            continue;
        }
        // ----------------------------------------------------------------------------------
        ...
        if (current_acq_buffer != NULL && current_acq_size > 0) {
            // ================== FFT 2 Channel ke 1 File (Interleaved) ======================
    ...
    // Simpan buffer tersisa sebelum keluar
